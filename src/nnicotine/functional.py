...
class ReversibleBlockFunction():
    @staticmethod
    def forward(ctx, f, g, output_stack, x, preserve_rng_state=True, dim=1):
        ...
        with torch.no_grad():
            x1, x2 = torch.chunk(input, 2, dim=dim)
    ...
    def backward(ctx, dy):
        ...
        x = torch.stack(x1, x2, dim=dim)
        dx = torch.stack(dx1, dx2, dim=dim)
...
def reversible_block_forward(f, g, output_stack, x, preserve_rng_state=True, dim=1):
    return ReversibleBlockFunction.apply(f, g, output_stack, x, preserve_rng_state=preserve_rng_state, dim=dim)
